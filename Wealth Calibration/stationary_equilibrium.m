...
function [Y,C,S,K_eq,L,lump_sum,r_eq,w_eq,g_final,a_marg_dist] = ...
    stationary_equilibrium(calibration,parameters,specification,a_grid)
...
% 1.2 - Calibration Moment Matching
delta_rho = calibration(1);
rho = calibration(2);
borrowing_limit = calibration(3);
...
if specification.closed_economy == 1
        ...
        if specification.disc_rate_heterogeneity == 1
                ...
                if specification.ind_fixed_effects == 0
                    ...
                    for i=1:n_fe
                        [x,y,z] = HJB(parameters,omega_grid(i), ...
                            a_grid,borrowing_limit,r_iter,w,lump_sum,rho_j);
                        g_cell{i} = x;
                        con_cell{i} = y;
                        sav_cell{i} = z;
                    end
            ...
            if specification.ind_fixed_effects == 0
                [g_final,con_final,~] = HJB(parameters,0,a_grid, ...
                    borrowing_limit,r_iter,w,lump_sum);
            else
                g_cell = cell(n_fe);
                con_cell = cell(n_fe);
                sav_cell = cell(n_fe);
                ...
                for i=1:n_fe
                    [x,y,z] = HJB(parameters,omega_grid(i), ...
                        a_grid,borrowing_limit,r_iter,w,lump_sum);
                    g_cell{i} = x;
                    con_cell{i} = y;
                    sav_cell{i} = z;
                end
    ...
    if specification.ind_fixed_effects == 0
        [g_final,con_final,~,~] = HJB(parameters,0,a_grid, ...
                                            borrowing_limit,r_open_econ,w,lump_sum);
    else
        g_cell = cell(n_fe);
        con_cell = cell(n_fe);
        sav_cell = cell(n_fe);
        for i=1:n_fe
            [x,y,z] = HJB(parameters,omega_grid(i), ...
                                a_grid,borrowing_limit,r_open_econ,w,lump_sum);
            g_cell{i} = x;
            con_cell{i} = y;
            sav_cell{i} = z;
        end
