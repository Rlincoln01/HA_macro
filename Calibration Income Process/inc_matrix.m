...
function [TM_y,ygrid,TM_p,p_grid,TM_t,t_grid] = inc_matrix(parameters,settings)

% ========== Settings ========== %

n_t = settings.n_t;
n_p = settings.n_p;
pgrid_width = settings.pgrid_width;
tgrid_width = settings.tgrid_width;
tgrid_par = settings.tgrid_par;
pgrid_par = settings.pgrid_par;

% ========== Parameters ========== %

% Parameters of the income process
beta_t = parameters(1);
beta_p = parameters(2);
sigma_t = parameters(3);
sigma_p = parameters(4);
lambda_t = parameters(5);
lambda_p = parameters(6);
